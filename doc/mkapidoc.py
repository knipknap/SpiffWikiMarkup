...
doc_dir  = 'api'
doc_file = os.path.join(doc_dir, 'spiff_wikimarkup.py')
files    = ['../src/SpiffWikiMarkup/Wiki2Html.py',
            '../src/SpiffWikiMarkup/Html2Wiki.py'] # Order matters - can't resolve inheritance otherwise.
classes  = [os.path.splitext(os.path.basename(file))[0] for file in files]
classes  = ['(?:SpiffWikiMarkup.)?' + cl for cl in classes]
